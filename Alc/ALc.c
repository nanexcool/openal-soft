...
static const ALCenums enumeration[] = {
    ...
    DECL(ALC_EXTENSIONS),
    DECL(ALC_FREQUENCY),
    DECL(ALC_UPDATE_SIZE),
    DECL(ALC_UPDATE_BUFFERS),
    DECL(ALC_REFRESH),
    DECL(ALC_SYNC),
...
static ALCenum UpdateDeviceParams(ALCdevice *device, const ALCint *attrList)
{
    ...
    ALCuint oldFreq, oldUpdateSize, oldNumUpdates;
    FPUCtl oldMode;
    ALuint i;
    ...
    {
        ALCuint freq, updateSize, numUpdates, numMono, numStereo, numSends;
        ALCuint attrIdx = 0;
        ...
        while(attrList[attrIdx])
        {
            ...
            if(attrList[attrIdx] == ALC_UPDATE_SIZE)
            {
                updateSize = attrList[attrIdx + 1];
                device->Flags |= DEVICE_UPDATE_SIZE_REQUEST;
            }

            if(attrList[attrIdx] == ALC_UPDATE_BUFFERS)
            {
                numUpdates = attrList[attrIdx + 1];
                device->Flags |= DEVICE_UPDATE_BUFFERS_REQUEST;
            }
    ...
    oldUpdateSize = device->UpdateSize;
    oldNumUpdates = device->NumUpdates;
    ...
    if(device->UpdateSize != oldUpdateSize && (device->Flags&DEVICE_UPDATE_SIZE_REQUEST))
    {
        ERR("Failed to set %u, got %u instead\n", oldUpdateSize, device->UpdateSize);
        device->Flags &= ~DEVICE_UPDATE_SIZE_REQUEST;
    }
    if(device->NumUpdates != oldNumUpdates && (device->Flags&DEVICE_UPDATE_BUFFERS_REQUEST))
    {
        ERR("Failed to set %u, got %u instead\n", oldNumUpdates, device->NumUpdates);
        device->Flags &= ~DEVICE_UPDATE_BUFFERS_REQUEST;
    }
...
ALC_API ALCvoid ALC_APIENTRY alcGetIntegerv(ALCdevice *device,ALCenum param,ALsizei size,ALCint *data)
{
    ...
    if(!device)
    {
        switch(param)
        {
            ...
            case ALC_FREQUENCY:
            case ALC_REFRESH:
            case ALC_UPDATE_SIZE:
            case ALC_UPDATE_BUFFERS:
            case ALC_SYNC:
        ...
        switch(param)
        {
            ...
            case ALC_ALL_ATTRIBUTES:
                ...
                else
                {
                    ...
                    if(device->Type != Loopback)
                    {
                        data[i++] = ALC_UPDATE_SIZE;
                        data[i++] = device->UpdateSize;

                        data[i++] = ALC_UPDATE_BUFFERS;
                        data[i++] = device->NumUpdates;
